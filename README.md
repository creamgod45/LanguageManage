...
- 建立互相隔離的語言管理方案，每個方案擁有自己的檔案清單與快取。
...
2. 點選「新增方案」，明確選取要列管的 JSON、YAML/YML 或 PHP 語言檔。
3. 輸入方案名稱；插件完成解析後會顯示 JOIN 翻譯表與問題清單。
...
| 檔案 | 關鍵資訊 |
| --- | --- |
| `LocalizationModels.kt` | `LanguageSchemeDto`、`LanguageEntryDto`、`LanguageIssueDto`、狀態、mutation 與 Diff preview DTO |
...
| `localization/LocalizationManagerPanel.kt` | 主要 UI；方案列、搜尋列、翻譯表、問題表、剪貼簿、Diff 與操作事件 |
...
| `LanguageFileSupport.kt` | 安全路徑驗證、UTF-8 讀寫、原子寫入、JSON/YAML/PHP parse/render |
...
1. 使用者必須透過 file chooser 明確選取語言檔。
2. Backend 對每個路徑執行安全驗證、去重並持久化方案。
3. `loadScheme()` 計算每個檔案的 `lastModifiedTime XOR size` fingerprint。
4. Cache format 與 fingerprints 都相符時直接載入 `cache-{schemeId}.json`。
5. Cache miss 時逐檔 parse，先發布 entries 與 parser issues，讓 UI 不必等待使用率掃描。
6. Backend 最多掃描 2,000 個、每個不超過 512 KB 的來源檔，計算 key 使用次數。
7. 執行品質分析、更新磁碟 cache，再透過 `StateFlow` 推送完整狀態。
