...
- Rename the active scheme from **Scheme Settings** without changing its managed files or isolated identity. Maintain up to 1,000 file/directory exclusions per scheme, bulk-add comma/newline input, or use the mixed multi-selection **Choose Files/Folders** picker. A bare name matches files or directories at any depth; a relative path targets one file or a directory subtree. The Project file tree shortcut accepts mixed selections and is disabled when no scheme is active.
- Extend an existing scheme at any time with **Add Tracked Files**: mixed file/folder selections are safely parsed and shown in a selectable recognition table before they are added. **Operations → Add Namespace Files** can create and track one empty PHP/Properties namespace file per locale; **Delete Namespace Files** previews and removes that namespace family from disk and tracking as a rollback-capable transaction.
- Turn an editor selection into a managed translation through **Localization Manager → Create Translation from Selection**. Optional ordered `%key%` template conditions scan only the active scheme working directory with its exclusions. A cancellable JetBrains background task and modal progress bar report the current stage, directory/file/match counters, and relative path; the lazy result list leads to a final multi-file confirmation where source-code results are editable and generated language-file changes remain read-only.
...
- Cover runtime-built translation keys with two localization dynamic usage-source workflows. Editor comment markers such as `@languageManager(method: dynamic, enum: auth.failed,status.ready)` are discovered during the normal scheme scan; the non-invasive **Localization Dynamic Usage Sources** Tool Window tab stores repeatable file/line/column and key groups without changing source code. An editor caret can create either form without selecting text, while an optional selection prefills keys; both forms contribute to the same usage counts and locations.
- Both dynamic-source editors support scrollable repeatable groups and bulk key paste using newline, comma, semicolon, or a custom literal delimiter. Hovering a valid `@languageManager` marker shows its documentation; use **Edit Localization Dynamic Usage Marker** from the editor context menu or floating code toolbar to edit copied or existing markers.
- Convert either dynamic-source representation into the other from its editor form. Conversion updates the source file and scheme rule together with rollback, VFS/document reload, cache invalidation, and recounting. Gutter icons mark both types in open editors: marker icons navigate in code, while non-invasive icons open and focus the corresponding Tool Window rule.
- Valid non-invasive rules also render a lightweight block inlay on the line above the target, aligned with the registered code position. It shows the distinct dynamic-key count, exposes rule details on hover, and opens the exact rule when clicked; stale coordinates are not inserted into unrelated source text.
...
- Cache parsed state and dynamic usage locations in memory and under `.idea/language-manager/` to reduce repeated parsing. Saving dynamic rules invalidates the active scheme cache and starts a background reload; first load, manual reload, scheme switching, and imported schemes all use the dynamic definitions.
...
| `discoverAdditionalLanguageFiles(...)` / `addTrackedFiles(...)` | Previews mixed file/folder parser results, then validates and appends selected recognized files to an existing scheme | No; writes plugin scheme data only |
...
| `previewNamespaceFiles(...)` / `createNamespaceFiles(...)` | Derives one target per locale from a tracked PHP/Properties structure reference, previews it, then creates and tracks the accepted files | Preview: no; create: yes |
| `previewDeleteNamespaceFiles(...)` / `deleteNamespaceFiles(...)` | Previews one PHP/Properties namespace family, verifies hashes, then transactionally removes its files and tracking metadata | Preview: no; delete: yes |
...
- PHP parses strings, numbers, booleans, nested arrays, static string concatenation, and multiline heredoc/nowdoc values inside `return [...]` or `return array(...)`. For example, `'first'."\n\n".'second'` becomes one translation value. A heredoc/nowdoc delimiter is not fixed to `TEXT`; any matching valid PHP identifier such as `EOT`, `HTML`, or `MESSAGE_2026` works with `<<<LABEL`, `<<<"LABEL"`, or `<<<'LABEL'`. The identifier only marks the string boundary and does not determine its content type. Variables, interpolation, functions, and arbitrary expressions are rejected and never executed. When files are grouped below a language directory, `en/components/pagination.php` is identified as locale `en` and namespace `components.pagination`.
