...
internal object SchemeSettingsTransferSupport {
    ...
    fun export(schemes: List<LanguageSchemeDto>, projectBasePath: String?): String {
        require(schemes.isNotEmpty()) { backendMessage("scheme.transfer.export.empty") }
        val root = projectBasePath?.takeIf(String::isNotBlank)?.let(Path::of)?.toAbsolutePath()?.normalize()
        val portable = schemes.map { scheme ->
            PortableLanguageSchemeDto(
                name = scheme.name,
                files = scheme.files.map { portablePath(it, root) },
                usageScanSettings = scheme.usageScanSettings.copy(
                    basePath = scheme.usageScanSettings.basePath.takeIf(String::isNotBlank)?.let { portablePath(it, root) }.orEmpty(),
                ),
            )
        }
    ...
    fun preview(content: String, projectBasePath: String?): SchemeImportPreviewDto {
        ...
        return SchemeImportPreviewDto(
            ...
            schemes = transfer.schemes.map { scheme ->
    ...
    fun resolve(content: String, projectBasePath: String?): List<ResolvedImportedScheme> {
        val transfer = decode(content)
        val root = requireProjectRoot(projectBasePath)
        return transfer.schemes.map { scheme ->
            val files = scheme.files.map { configured ->
            ...
            val basePath = scheme.usageScanSettings.basePath.takeIf(String::isNotBlank)?.let { configured ->
    ...
    private fun decode(content: String): SchemeSettingsTransferDto {
        ...
        val transfer = runCatching { json.decodeFromString<SchemeSettingsTransferDto>(content) }
    ...
    private fun inspect(configured: String, root: Path): SchemeImportFilePreviewDto {
        val resolved = runCatching { resolveConfiguredPath(configured, root) }.getOrElse { error ->
    ...
    private fun resolveConfiguredPath(configured: String, root: Path): Path {
    ...
    private fun portablePath(raw: String, root: Path?): String {
    ...
    private fun safeDetail(error: Throwable): String = (error.message ?: error.javaClass.simpleName)
        .replace(Regex("[\u0000-\u001F\u007F-\u009F]"), "?")
        .take(500)
}
