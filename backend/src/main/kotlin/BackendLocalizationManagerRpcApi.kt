...
class BackendLocalizationManagerRpcApi : LocalizationManagerRpcApi {
    ...
    override suspend fun createScheme(
        ...
        projectId.service()?.createScheme(name, files, usageSettings)
        Unit
    }

    override suspend fun deleteScheme(
        projectId: ProjectId,
        schemeId: String,
    ) = withContext(Dispatchers.IO) {
        projectId.service()?.deleteScheme(schemeId)
        Unit
    }

    override suspend fun activateScheme(
        projectId: ProjectId,
        schemeId: String,
    ) = withContext(Dispatchers.IO) {
        projectId.service()?.activateScheme(schemeId)
        Unit
    }
    ...
    override suspend fun reload(
        ...
        projectId.service()?.reload(schemeId, force)
        Unit
    }
    ...
    override suspend fun previewSchemeSettingsImport(
        projectId: ProjectId,
        content: String,
    ...
    override suspend fun importSchemeSettings(
        projectId: ProjectId,
        content: String,
    ) = withContext(Dispatchers.IO) {
        projectId.service()?.importSchemeSettings(content)
        Unit
    }
    ...
    override suspend fun saveEntry(
        ...
        projectId.service()?.saveEntry(schemeId, mutation)
        Unit
    }
    ...
    override suspend fun deleteEntries(
        ...
        projectId.service()?.deleteEntries(schemeId, entryIds)
        Unit
    }
    ...
    override suspend fun repair(
        ...
        projectId.service()?.repair(schemeId)
        Unit
    }

    override suspend fun repairEntries(
        projectId: ProjectId,
        schemeId: String,
        entryIds: List<String>,
    ) = withContext(Dispatchers.IO) {
        projectId.service()?.repairEntries(schemeId, entryIds)
        Unit
    }

    override suspend fun previewLocaleVersion(
        projectId: ProjectId,
        schemeId: String,
        request: LocaleVersionRequestDto,
    ): ChangePreviewDto = withContext(Dispatchers.IO) { projectId.service()?.previewLocaleVersion(schemeId, request) ?: ChangePreviewDto() }

    override suspend fun createLocaleVersion(
        projectId: ProjectId,
        schemeId: String,
        request: LocaleVersionRequestDto,
    ...
    override suspend fun previewChanges(
        projectId: ProjectId,
        schemeId: String,
        request: ChangePreviewRequestDto,
    ): ChangePreviewDto = withContext(Dispatchers.IO) { projectId.service()?.previewChanges(schemeId, request) ?: ChangePreviewDto() }

    override suspend fun applyPreviewedChanges(
        projectId: ProjectId,
        schemeId: String,
        request: ChangePreviewRequestDto,
        expectedBeforeHashes: Map<String, String>,
    ) = withContext(Dispatchers.IO) {
        projectId.service()?.applyPreviewedChanges(schemeId, request, expectedBeforeHashes)
        Unit
    }
}
