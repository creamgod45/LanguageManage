...
internal class SchemeUsageSettingsDialog(
    ...
    private val filesArea = JBTextArea(scheme.files.joinToString("\n"), 4, 48).apply {
        isEditable = false
        lineWrap = false
    }
    ...
    fun result(): UsageScanSettingsDto = UsageScanSettingsDto(
    ...
    override fun createCenterPanel(): JComponent {
        val basePathPanel = JPanel(BorderLayout(JBUI.scale(5), 0)).apply {
        ...
        val panel = FormBuilder.createFormBuilder()
            .addLabeledComponent(message("settings.scheme.files"), JBScrollPane(filesArea).apply {
            ...
            .addLabeledComponent(
                ...
                listEditor(exclusionModel, message("settings.exclusion.add.prompt"), message("settings.exclusion.edit.prompt"), DEFAULT_USAGE_EXCLUDED_DIRECTORIES),
            )
    ...
    private fun chooseBasePath() {
        ...
        FileChooserFactory.getInstance().createFileChooser(descriptor, dialogProject, null)
            .choose(dialogProject).firstOrNull()?.let { basePathField.text = it.path }
    }
        ...
        return JPanel(BorderLayout(0, JBUI.scale(4))).apply {
            ...
            add(JPanel(FlowLayout(FlowLayout.LEADING, JBUI.scale(4), 0)).apply {
                add(JButton(message("settings.list.add")).apply {
                    addActionListener {
                        Messages.showInputDialog(dialogProject, addPrompt, title, null)
                            ?.trim()?.takeIf(String::isNotEmpty)?.let { value ->
                ...
                add(JButton(message("settings.list.edit")).apply {
                    addActionListener {
                        ...
                        Messages.showInputDialog(dialogProject, editPrompt, title, null, model[index], null)
