...
- Add diagnostics for a JetBrains diff performance issue: rendering a Markdown (`.md`) source file in the rename preview with code folding can spike CPU. The preview now logs the diffed file, resolved file type, content sizes, and per-render wall-clock time, and warns when a single render is slow. The suspected platform Markdown folding hotspot is marked with `@todo(diff-markdown-cpu)` comments at the diff-content and `setRequest` call sites for follow-up.
