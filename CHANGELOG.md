...
- Fix high CPU when the rename preview diffs a large Markdown (`.md`) source file: the JetBrains diff viewer re-runs Markdown code folding and fenced-code injection on every dropdown switch. Oversized Markdown content is now downgraded to plain text for the diff so the platform skips that work, while small Markdown keeps full highlighting. The preview also logs the diffed file, resolved file type, content sizes, and per-render wall-clock time, warns when a single render is slow, and marks the hotspot with `@todo(diff-markdown-cpu)` comments for follow-up.
