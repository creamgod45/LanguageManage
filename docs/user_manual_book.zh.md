...
Tool Window 有兩個同級內容：「語言方案」負責翻譯管理，「分析」負責高成本專案檢查。停留在語言方案時不會執行分析；只有使用者手動切換到「分析」才會掃描作用中方案，也可按「重新分析」更新結果，按「停止」則取消 JetBrains 背景任務。

「未翻譯硬編碼文字」分析會掃描作用中方案 base path、遵守檔案／資料夾排除規則、略過列管語言檔，而且不額外限制副檔名。已被方案使用率 Regex 辨識的程式碼範圍不列入候選。每個位置都以獨立的 `Value / File path / Line / Col` row 顯示，可搜尋、依可信度篩選，或分別勾選純英文單字、camelCase、PascalCase／UpperCamelCase、snake_case、MACRO_CASE、kebab-case、dot.case，以及 `$name`、`${name}`、`{{ $name }}`、`{!! $html !!}` 等 PHP／Blade 變數排除條件；包含屬性、函式、索引或前後綴文字的 Blade 運算式也會排除。多選採聯集即時更新表格，不會重新掃描。每頁最多 100 列並可雙擊導航；「匯出結果」會套用目前搜尋文字、可信度與命名格式排除條件，將所有符合頁面輸出為 UTF-8 CSV，按鈕會同步顯示筆數。上方分別統計掃描、快取、略過、命中、候選、唯一文字及高／中／低可信度數量。這些是需要人工評估的啟發式候選，不會直接視為錯誤，也不會自動修改檔案。
...
日後要擴充既有方案時，在方案下拉選單選擇「**增加追蹤檔案**」。可反覆使用「新增檔案」與「新增資料夾」先建立完整路徑清單；分別選取 `en`、`zh_TW` 時兩個根目錄都會保留、掃描並合併。檢查 parser 辨識表後再勾選要加入的檔案；方案 ID、設定及原有追蹤檔案都會保留。

PHP 或 Properties 的多檔分類可使用「**操作 → 新增 Namespace 檔案**」。先選一個代表目標目錄家族的列管檔，輸入 `components.filament` 之類的點分隔 Namespace，透過 Diff 確認每個語言將建立的檔案後再套用。插件不會覆寫既有目標，確認的檔案會一起建立並加入追蹤。

要移除檔案型 Namespace 時，使用「**操作 → 刪除 Namespace 檔案**」。選擇一個列管檔作為家族參考，先檢查各語言完整刪除內容的 Diff；檔案未變更才會從硬碟與方案追蹤清單一起移除，交易失敗則還原暫移檔案。此操作不允許刪除方案最後一組追蹤檔。
5. 在獨立名稱欄位輸入或修改容易辨識的方案名稱，例如「網站前台」或「Admin API」，確認要列管的可識別檔案後點擊「建立方案」。名稱會去除前後空白，且必須為 1–80 個不含控制字元的字元。
...
1. 先啟用擁有目標語言檔的方案，在編輯器選取一個字或一段文字後按右鍵，選擇「**在地化管理器 → 以選取範圍建立翻譯文字**」。沒有選取範圍或目前方案時，此操作會停用。
2. 輸入 key 並確認選取的原文。Modal 會在同一個可捲動表單列出目前 namespace 的所有 locale；有 `en` 時會先把選取文字帶入 `en`，否則使用第一個 locale。
3. 如要找出重複原文，可勾選「掃描類似文字」。每個條件需要一個 `%key%` placeholder 與一個檔名副檔名，並可持續新增。例如 `.php` 使用 `__('%key%')`，或 `.blade.php` 使用 `@lang('%key%')`。框架建議只會填入這兩個一般欄位，不會產生 Regex。
4. 條件完全保留使用者新增順序。每個檔名會採用由上到下第一個符合的副檔名；即使後面的條件更精準也不會自動提升優先權，因此畫面 row 順序就是實際執行順序。
5. 點擊「預覽符合項目」會啟動可取消的 JetBrains 背景任務。單次目錄規劃期間，IDE 任務與 Modal 進度條會顯示目錄／檔案／符合副檔名數，以及相對於方案 base path 的目前位置；取得精確符合檔案總數後，內容搜尋階段會切換為 determinate 進度，顯示「已搜尋／總數」、百分比與命中數，且不會重複走訪專案。可用旁邊的「停止掃描」同時取消 IDE 任務與 backend RPC；未完成的候選不會當成完整結果顯示。掃描只限目前方案工作目錄，並略過列管語言檔、二進位檔、無效 UTF-8 與超過 5 MB 的程式碼檔案；單次最多走訪 100,000 個檔案、讀取 50,000 個符合副檔名檔案、列出 2,000 個符合檔案。
6. 候選清單採 lazy 流程；雙擊檔案時才計算並開啟該檔案 Diff，Diff 標籤會以 `>>>{內容}<<<` 標示搜尋原文與產生的替換。核取方塊獨立控制哪些檔案進入最後操作；切換反白預覽 row 或雙擊預覽不會取消其他已勾選檔案。
7. 確認 Modal 後會進入最終多檔 Diff。檔案選擇器會顯示完整位置及「可編輯／唯讀」狀態：產生的語言檔結果維持唯讀，程式碼替換結果的右側可直接調整。
8. 點擊「套用」時，backend 會重新產生預覽、核對完整檔案集合與每個原檔 SHA-256、驗證已編輯內容大小，再以具回復機制的原子寫入完成。取消任一預覽都不會寫入檔案。

替換樣板只會被當作字串資料，不會編譯成 Regex、不會 eval PHP，也不會作為指令執行；選取原文採純文字搜尋。

如需回報掃描過慢，請一併提供 IDE log 中包含 `Selection replacement scan` 的行。這些紀錄會顯示開始／進度／完成／取消／失敗統計與耗時，但不包含選取原文或替換樣板。
...
- 每頁最多 100 列。
- 使用表格下方「上一頁」與「下一頁」切換。
- 手動／背景重新讀取，以及新增、編輯、刪除、Key 改名、AI 翻譯、修復或正規化後的狀態更新，都會保留目前頁碼；剩餘頁數變少時會移至最後一個有效頁面。
- 搜尋文字、精準／模糊模式、語言或翻譯狀態篩選改變時會回到第一頁。
...
2. 點擊「Key 改名」。
3. 輸入新 key。
4. 不勾選「同步修改已記錄使用位置中的程式碼 key」時，維持原本只修改語言檔的流程。
5. 勾選後會依所選 row 的快取使用位置建立預覽；語言檔差異保持唯讀，符合的程式碼檔案可直接編輯右側結果。
6. 檢查每個檔案後按「套用」才會寫入；取消會關閉預覽且不修改語言檔或程式碼。

Backend 只接受快取位置仍包含精確 key、點分隔 namespace 引用或 Laravel 斜線群組引用的紀錄，並保留 vendor 前綴；來源檔已變更、超出方案工作目錄或超過大小限制時會拒絕處理。套用前會重新產生預覽並比對 SHA-256。若任一語言檔已有新 key，整個操作會拒絕寫入以避免覆蓋。

### 合併翻譯

1. 恰好選取兩筆翻譯 row，點擊「操作 ▾ → 合併翻譯」。
2. 確認哪一筆是合併後移除的來源、哪一筆是保留的目標；需要時可按「交換來源與目標」。
3. 既有且非空白的目標語系值優先保留；目標缺少或空白時，先由相同語系的來源補值，再移除來源 key。
4. 啟用程式碼同步時，檢查可編輯的目標引用。Laravel 內部 namespace `admin.customer` 預設轉成 `admin/customer.key`；vendor 或 `Lang::addNamespace()` 自訂別名可填成 `package::admin/customer.key`。
5. 在最後 Diff 檢查所有語言檔及程式碼檔。語言檔為系統產生的唯讀結果，符合的程式碼檔右側可編輯；按「套用」後才會驗證雜湊並完成整筆交易。

只有最近一次方案掃描記錄到的使用位置會被修改。更新器可辨識完整點分隔引用、Laravel 斜線群組、vendor 前綴，以及只擷取 key 的位置；若某個來源語系找不到唯一的目標 namespace 檔，或掃描後來源檔已變更，會停止整次合併而不產生部分寫入。
...
在 LanguageManager Tool Window 上方先選擇方案，再點擊「方案設定」。Popup 會直接使用 Tool Window 已載入的目前方案，可重新命名並顯示列管語言檔案與獨立掃描設定。改名不會變更方案 ID、列管檔案、快取歸屬或隔離狀態，也不會再從 IDE Settings 頁動態讀取方案。
...
針對執行期組合 key，將編輯器游標放在來源位置，再使用「**在地化管理器 → 建立在地化管理動態使用來源**」。不必選取文字；若選取一個或多個 key 會自動預填，未選取時則依游標行／欄開啟可自行輸入的空白 key row：

- 「**建立程式碼備註標記**」會開啟可重複新增命名群組與 key row 的小表單，確認後插入例如 `// @languageManager(method: dynamic, enum: custinfo,packinfo)`。掃描器只接受位於程式碼備註中的標記，並將標記位置記錄為使用位置。
- 「**新增非侵入式動態來源**」會開啟不搶焦點的 Tool Window「**動態來源**」頁籤，預填目前檔案、行、欄與選取 key。可自由新增／移除外層規則、命名群組及內層 key row；key 右側按鈕會列出目前翻譯表建議。此方式不會修改程式碼。
- 兩種編輯器都能捲動。使用「**批次加入 Key**」可貼上以換行、逗號、分號或自訂純文字分隔符切割的 Key；系統保留輸入順序並移除重複值，每個群組最多 500 個 Key。
- 將游標懸停在有效的 `@languageManager` 標記可查看語法與已宣告群組。把游標放進既有或複製的標記，再從「**在地化管理器 → 建立在地化管理動態使用來源**」或浮動式代碼工具欄選擇「**編輯在地化動態使用標記**」；表單只會替換標記本體並保留外層備註。
- 在 marker 編輯表單選擇「**轉換為非侵入式規則**」，會移除經驗證的備註 marker，並在相同來源位置建立方案規則。非侵入式規則卡片的「**轉換為程式碼備註標記**」會依檔案類型在登錄行插入備註，並移除該規則；目前尚未另外儲存的表單值也會一併套用。檔案與方案採同一個可回復操作，成功後重新載入 IDE 文件、清除使用率快取並重算目前方案。
- 已開啟的來源編輯器會在侵入式 marker 行與非侵入式登錄行顯示在地化管理器裝訂區圖示。點擊侵入式圖示會前往 marker；點擊非侵入式圖示會開啟「**動態來源**」頁籤並捲動到確切規則卡片。若方案狀態尚在傳入，系統會保留定位請求直到規則完成顯示。
- 有效的非侵入式位置也會在目標程式碼上一行顯示類似參數名稱提示的 `在地化管理: N個鍵` block inlay，並與儲存的程式碼位置對齊。懸停可查看方法、檔案位置、群組與 key；點擊可定位到確切規則卡片。若編輯造成儲存座標失效，系統不會把提示強行放進編輯器，裝訂區 Tooltip 會要求檢查規則。
- 每張非侵入式規則卡片也提供「**前往標記位置**」，會依表單目前顯示的檔案、行、欄開啟來源位置，因此修改座標後不必先儲存即可確認位置。
- 「**搜尋翻譯表 → 精準搜尋／模糊搜尋**」會開啟 Tool Window，直接以編輯器選取文字搜尋。

編輯非侵入式規則後按「**儲存動態來源**」。來源路徑必須是方案使用率 base path 內的一般本機檔案；URI／device path、控制字元、根目錄外檔案、無效 key 及超量規則／群組／key 都會被拒絕。規則以方案隔離，並會包含於方案設定匯入／匯出。

Regex 命中、程式碼備註標記及已儲存的非侵入式規則會合併為同一份使用次數／位置結果。首次解析、手動「重新讀取」、需要重建快取的方案切換及匯入方案載入都會套用；儲存規則會清除該方案快取並立即排入背景重讀。來源路徑與 offset 會快取，line／column 仍等使用者開啟位置時才計算。
...
「使用位置」Tab 預設停用且保持空白，一般方案載入不會替未互動的位置資料建立 Swing table。位置紀錄只保留於 backend，不再包含於一般狀態 Flow。雙擊某一翻譯 row 的「使用次數」cell，才會選取該 `namespace + key`、啟用 Tab，並向 backend 請求該 key 的當頁資料。表格包含來源檔、行號、欄位與命中次數，每個 RPC 分頁最多 100 筆。初始行／欄顯示「開啟時計算」：掃描階段只快取字元 offset 與來源檔修改時間；雙擊位置或按「開啟位置」後，backend 才計算行／欄、更新快取並把 IDE 游標移到該處。若來源檔在掃描後變更，插件會拒絕舊 offset 並要求重新讀取。
...
- 可在方案設定手動輸入名稱／相對路徑、大量貼上，或使用「選擇檔案／資料夾」同時多選檔案與資料夾。單一名稱會排除任意層級的同名檔案或資料夾；相對檔案路徑只排除該檔案，相對資料夾路徑則排除整個子樹。
- 在 Project 檔案樹混合選取一個或多個檔案／資料夾，使用「在地化語言管理 → 從目前方案掃描中排除檔案／資料夾」。插件會儲存相對於目前方案 base path 的精確路徑，接著使 cache 失效並重新計算。沒有啟用方案時操作會停用；掃描根目錄、根目錄外、不安全及超量項目會拒絕或回報，但不會中止同批其他有效項目。
...
- 點擊「重新讀取」會強制重新解析。
- 列管語言檔或已設定的動態來源檔變更、cache format 升級或 fingerprint 不符時會重建 cache；其他掃描來源檔的變更可使用手動「重新讀取」取得。
...
- 每次內容 mutation 成功後，插件會在最後只針對實際寫入的語言檔／程式碼檔，同步 refresh JetBrains VFS。
- 如果目標檔案已有 IDE 快取 document，完整交易成功後會呼叫 IDE 的「從硬碟重新載入」流程，載入最終內容。
- 方案與 cache 中繼檔刻意排除；若編輯器仍不同，請確認是否另有未儲存的編輯內容，或其他程序在插件完成後再次寫檔。
...
每種 Issue 類型都會分開顯示 **English** 與**繁體中文**兩份模板。請選擇要填寫的語言卡片；同一份表單不會混合顯示兩種語言。
