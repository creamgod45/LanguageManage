...
5. 點擊「預覽符合項目」只會掃描目前方案工作目錄，並遵守排除清單、略過列管語言檔、二進位檔、無效 UTF-8 與超過 5 MB 的程式碼檔案。單次最多檢查 50,000 個檔案、列出 2,000 個符合檔案。
6. 候選清單採 lazy 流程；雙擊檔案時才計算並開啟該檔案 Diff，Diff 標籤會以 `>>>{內容}<<<` 標示搜尋原文與產生的替換。可選取或取消候選檔案，控制哪些項目進入最後操作。
