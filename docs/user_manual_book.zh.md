...
Backend 只接受快取位置仍精確包含 `oldKey` 或 `namespace.oldKey` 的紀錄，不確定的外層前綴會原樣保留；來源檔已變更、超出方案工作目錄或超過大小限制時會拒絕處理。套用前會重新產生預覽並比對 SHA-256。若任一語言檔已有新 key，整個操作會拒絕寫入以避免覆蓋。
