...
- 可透過 OpenAI-compatible 或 Anthropic Claude 端點，一次 AI 翻譯最多 100 個已選 row；API Token 儲存在 JetBrains PasswordSafe。產生結果可先編輯，再進入檔案層級 Diff；只有「套用」會寫檔，「提出其他意見 AI」會把來源值、已檢視建議與意見帶入新一輪請求。
...
| `AiTranslationSupport.kt` | 驗證端點與批次限制、送出 OpenAI-compatible／Anthropic 請求，並嚴格驗證回傳 ID 與 value |
...
| `translateWithAi(...)` | 將暫時性 Provider 憑證與最多 100 個來源值傳到指定端點，只回傳翻譯建議 | 否 |
