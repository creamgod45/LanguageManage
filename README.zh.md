...
目前介面與診斷支援英文、繁體中文、簡體中文、日文、韓文、西班牙文及泰文；顯示語言預設跟隨 IDE，也可在插件設定中明確指定。
...
- [使用者操作手冊](docs/user_manual_book.zh.md)：安裝、方案、翻譯表狀態篩選、問題顯示偏好、使用率掃描、Diff 與疑難排解。
- [需求規格](docs/需求.md)：功能需求、表格篩選、issue 顯示規則、RPC、安全、快取與驗收條件。
- [記憶體與大型方案驗證](docs/performance_memory.zh.md)：可重現的 12 語言解析、掃描、常駐記憶體與 RPC payload 實測。
- [專案開發提示](AGENTS.md)：程式原則、架構界線、多國語言、測試與 Git 習慣。
- [版本紀錄](CHANGELOG.md)
- Marketplace「What’s New」：建置時直接擷取 [CHANGELOG.md](CHANGELOG.md) 的目前版本區段並注入 `plugin.xml`，不另外維護重複摘要。
...
| 檔案 | 關鍵資訊 |
| --- | --- |
| `LocalizationModels.kt` | 方案、locale 備註、entry、issue、使用率預設、翻譯 row filter、AI request、資料夾識別結果、mutation 與 Diff preview DTO |
...
| `AiTranslationSupport.kt` | 驗證端點、locale 備註上下文與批次限制，送出 OpenAI-compatible／Anthropic 請求並嚴格驗證回傳 ID 與 value |
| `SchemeSettingsTransferSupport.kt` | 含 locale 備註的有版本方案 JSON 編解碼、相對路徑轉換、匯入上限與安全驗證 |
...
| `frontend/src/test/kotlin/ToolWindowIconVariantsTest.kt` | 四種 Tool Window 圖示的資源存在性、尺寸與主題色 |
| `backend/src/test/kotlin/UsageScanSupportTest.kt` | 自訂 Regex、排除相對路徑、計數結果與不安全設定拒絕 |
| `docs/compatibility.zh.md` | Marketplace／Plugin Verifier 已驗證的 IDE 版本與插件相容範圍 |
...
| `saveEntry(...)` | 新增／編輯單一 entry；保留給單一 cell 貼上與相容流程 | 是 |
| `saveEntries(...)` | 將可捲動翻譯表單中的全部 locale value 一次驗證並批量寫入 | 是 |
| `translateWithAi(...)` | 將暫時性 Provider 憑證、受限來源／目標 locale 備註與最多 100 個來源值傳到指定端點，只回傳翻譯建議 | 否 |
| `previewEntryMutations(...)` | 將翻譯 mutation render 為逐檔 before/after 與來源 SHA-256 | 否 |
| `applyPreviewedEntryMutations(...)` | 重建翻譯 mutation、驗證全部預覽 hash，並只對未變更來源執行 atomic write | 是 |
| `deleteEntries(...)` | 依 entry ID 批量刪除 | 是 |
| `renameKey(...)` | 在方案內所有包含舊 key 的檔案中改名 | 是 |
| `previewRenameKey(...)` | 驗證快取使用位置 offset，建立語言檔與程式碼 before/after，但不寫入 | 否 |
| `applyPreviewedRenameKey(...)` | 重建預覽、驗證 hash 與編輯後檔案集合，再原子寫入使用者確認的內容 | 是 |
...
7. Backend 依方案設定的 base path、Regex 與排除清單掃描根目錄下所有一般檔案，不以副檔名或 IDE 檔案類型額外過濾；方案列管語言檔本身會排除，避免把定義誤算成使用。
...
- JSON array 會展開為 `sections.0.items.3.title` 形式的一般翻譯列；每個項目與單一文字一樣支援 JOIN、搜尋、編輯、刪除、使用率、缺失檢查及 AI 翻譯，寫回時會重建原本的 array 結構。
- JSON 句子型 key 中的字面句點會透過 `keyPaths` 保留，不會誤建巢狀物件。
- YAML 支援以空白縮排的 `key: value` 與巢狀 map；不接受 tab 縮排。
- PHP 可解析 `return [...]` 或 `return array(...)` 中的字串、數字、布林值、巢狀 array、靜態字串串接及多行 heredoc／nowdoc；例如 `'第一段'."\n\n".'第二段'` 會合併為單一翻譯值。heredoc／nowdoc 識別字並非固定為 `TEXT`，`EOT`、`HTML`、`MESSAGE_2026` 等任何前後一致的合法 PHP identifier，搭配 `<<<LABEL`、`<<<"LABEL"` 或 `<<<'LABEL'` 都可使用；名稱只負責標示字串邊界，不代表內容類型。變數、interpolation、函式及任意 expression 仍會拒絕且絕不執行。語言目錄下依分類建立子目錄時，`en/components/pagination.php` 會辨識為 locale `en`、namespace `components.pagination`。
...
└── cache-{schemeId}.json     # fingerprints、entries、issues、使用次數與使用位置
```
...
插件最低支援 JetBrains Platform build `253.5`（IntelliJ IDEA 2025.3.5），未設定最高版本；已驗證版本請參閱 [相容性驗證](docs/compatibility.zh.md)。開發建置需求為 JDK 21，或使用 PhpStorm 2026.1 隨附 JBR。
...
- `backend`: JSON/YAML/PHP/Properties parse/render、安全路徑、array、語言目錄巢狀分類、句子型 key、使用率 Regex／排除計數、分析、唯讀 preview 與七語資源鍵完整性。
- `frontend`: 設定預設與遷移、issue 顯示過濾、Tool Window API 相容性與七語資源鍵完整性。

## 版本與授權

- 版本紀錄：[CHANGELOG.md](CHANGELOG.md)
- Marketplace「What’s New」：由 [CHANGELOG.md](CHANGELOG.md) 相同版本區段自動產生
