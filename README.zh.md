...
- 可將多個所選 key 一次複製到指定語言 value，並提供 PHP 主流框架、Spring／Java／Kotlin、ResourceBundle 與 JetBrains Plugin 的使用率 Regex 推薦格式。
...
- PHP 僅解析 `return [...]` 或 `return array(...)` 的字串、數字、布林值與巢狀 array，不執行函式或任意 expression。語言目錄下依分類建立子目錄時，`en/components/pagination.php` 會辨識為 locale `en`、namespace `components.pagination`。
